...
| 4 | **Native-speaker Arabic review — a worksheet of keys in `AR_GAPS` with no source in the bundle. NO COUNT IS QUOTED HERE ANY MORE, AND THAT IS THE POINT.** This row has carried **31**, then **74**, then **81**, then **84**, and each was wrong within days of being written — five stale figures for one list, which makes it the most-repeated instance of row 26's defect in this file. It grows whenever a slice adds a string the bundle has no Arabic for, which is most slices. **Measure it, do not read it:** count the quoted keys in the `AR_GAPS` array in `apps/wallet/src/copy/ar.ts` **after stripping its comments** — **86 on 2026-09-14**, stated as a dated measurement rather than a fact about the present. **Counting it is a trap of its own:** a naive count picks up apostrophes inside the array's own comments and reports 89. I did exactly that an hour before writing this, and caught it only by reconciling against a lane that had counted properly — so even the instruction to re-measure needs the stripping step spelled out. `i18n/digits.test.ts` asserts the array is EXACTLY the set of keys still holding English, so the LIST is guarded by a spec even while every prose count of it rots. | A person, with a lead time. The list is the worksheet. |
