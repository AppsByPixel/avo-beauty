...
| 40 | **Enforcing sign-in limits turned Lane D's e2e suite red, and the fix taken was an exemption whose cost is real.** Two suites share one identity across a long run — `console-reset.test.ts` signs one console handle in five times to prove a redeemed link is spent, and `signInDashboard`'s 15 static call sites share one salon-B handle over six minutes. Lane A exempted the limiter under `AVO_TEST_PRINCIPALS` (which `env.ts:356` refuses in production) rather than loosening the threshold to fit CI. **I verified the cost is exactly as disclosed: with the customer sign-in's charge line deleted, `pnpm check` stays 235 green.** The integration suite catches it — 8 specs red, including both indistinguishability ones — but that suite needs `AVO_INT_DATABASE_URL` and is not part of `pnpm check`. | So the guard for the most security-critical wiring in the API sits outside the default gate. The durable fix is in Lane D's column and Lane A correctly did not reach into it: cache one session per identity per run in `tenancy-harness.ts`, give `console-reset.test.ts` a fresh handle per round, then narrow or delete the exemption. Until then, "green check" and "the sign-in limiter is wired" are two different claims. |
