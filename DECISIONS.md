...
| 4 | **Native-speaker Arabic review — a worksheet of keys in `AR_GAPS` with no source in the bundle. NO COUNT IS QUOTED HERE ANY MORE, AND THAT IS THE POINT.** This row has carried **31**, then **74**, then **81**, then **84**, and each was wrong within days of being written — five stale figures for one list, which makes it the most-repeated instance of row 26's defect in this file. It grows whenever a slice adds a string the bundle has no Arabic for, which is most slices. **Measure it, do not read it:** count the quoted keys in the `AR_GAPS` array in `apps/wallet/src/copy/ar.ts` **after stripping its comments** — **105 on 2026-09-15** (86 the day before — the voucher slice added seventeen, and that rate is the point), stated as a dated measurement rather than a fact about the present. **Counting it is a trap of its own:** a naive count picks up apostrophes inside the array's own comments. On 2026-09-14 that made the true 86 read as 89, and I did exactly that an hour before writing this row, catching it only by reconciling against a lane that had counted properly. **On 2026-09-15 the naive and stripped counts agree, both 105** — because today's comments happen to contain no apostrophe. The trap fires or does not fire depending on prose, which is the worst kind of intermittent, so the stripping step stays in the instruction even on the days it changes nothing. `copy/ar.ts:954` carries the `awk` recipe. `i18n/digits.test.ts` asserts the array is EXACTLY the set of keys still holding English, so the LIST is guarded by a spec even while every prose count of it rots. | A person, with a lead time. The list is the worksheet. |
...
| 7 | **NARROWED 2026-09-13 — the engineering half was answered long ago, elsewhere, and this row never said so.** Sign-in needs a Workspace field, which is not in `AVO Login.dc.html`. Forced by `staff_user` being unique on `(salon_id, handle)`. **WHAT WAS NEVER OPEN: whether the field exists, and whether it may be conditional.** `design/api-contract.md:787` carries a block marked **"Ruling"** — *"merchant sign-in carries a workspace field and platform sign-in does not… forced by the data model rather than chosen"* — and *"carries"* is unconditional. **The contract even cites this queue row; this row cited nothing back.** So the answer sat one file away while the question was counted, published and presented as open — which is precisely the pathology row 8 records, found again in the row directly above it. **WHAT IT COST.** `SignIn.tsx` rendered the field as `{fromHost ? null : <TextField …/>}`, reading the departure as something to minimise. So **any** host yielding a hint deleted the control and silently posted the first DNS label as the salon id — a 401 with nothing on screen to correct. **Only `localhost` worked.** Not an exotic-host problem: `avo-dashboard.onrender.com`, `*.vercel.app` and every tunnel hit it identically, and so does **`dashboard.avo.beauty`, the host the design itself draws in the browser chrome of `AVO Login.dc.html`** — so the code comment claiming the hide *"matches the deployed shape and the design exactly"* was false about the design too. **And the case it was built for was broken as well, invisibly:** the subdomain label is `amara` while the salon id is `SAL-AMARA`, so the hint is wrong even at its best. The feature worked for nobody, anywhere, ever. Fixed in `b682462` — the hint is a DEFAULT, not a verdict — and confirmed live on a hosted domain. | **STILL AFTAB'S, and it is the only part that ever needed a human: whether the frozen design bundle is amended to draw the field.** A documentation question, not a blocker — the field ships either way, because the contract ruled it does. **The engineering lesson is about where answers live.** A ruling recorded in the contract did not reach the queue that asked for it, and the gap was invisible because both documents were individually correct. The eventual right shape is a public `GET /workspaces/{subdomain}` so a host can be RESOLVED rather than guessed — resolution is evidence, a DNS label is not — at which point hiding the field on a resolved host becomes legitimate again. That is an API change and is not queued here yet. |
...
| 22 | **RULED 2026-09-13 by Aftab, forced by taking both apps to App Store Connect: the wallet keeps `avo://`, the scanner takes `avostaff://`.** The reasoning is which app ACTS on each URI. `avo://topup/return` comes back to the customer after a payment and is already the API's `TOPUP_RETURN_URL` default, so the wallet keeps the brand scheme. `pay` moves to the scanner, because the scanner is what consumes it — and a customer's phone tapping such a link should find no handler rather than open her own wallet. **The blast radius turned out to be one line, and that is the payoff of an earlier fix.** `walletTokenUri()` in `packages/types/src/entities.ts:249` is the ONLY mint site: `api/src/routes/members.ts:1035` calls it rather than building the string, because the `uri` field was once stripped from the wire and every client re-derived it — *"two implementations of a bearer credential's format is one too many"*. So the API needs no change at all. The scanner's `domain/paymentCode.ts` holds the matching literal as `SCHEME`, and `paymentCode.test.ts` already round-trips the parser against `walletTokenUri()`, so the two halves cannot drift apart silently — changing one reddens the other by construction. **ORIGINAL FINDING.** Both mobile apps declare the same iOS URL scheme, `avo://`. `apps/wallet/app.json` and `apps/scanner/app.json` both set `"scheme": "avo"` (bundle ids differ: `beauty.avo.wallet`, `beauty.avo.scanner`). Driven on a device with both installed, iOS resolved `avo://pay?m=…&t=…` — the **payment** deep link, which belongs to the scanner — to **AVO Wallet**. Uninstalling the wallet made it resolve to the scanner correctly, which is how the QR charge path was finally verified. | Mostly benign in the pilot, because the customer's phone has the wallet and the counter phone has the scanner. It bites in three real cases and one of them is already scheduled: **(a)** a staff member who is also a customer of her own salon — likely — has both, and her scanner's deep-link path is hijacked; **(b)** the white-label pipeline (go-live: "one `--brand` token produces a per-salon app") would put N salon wallets on one phone all claiming `avo`; **(c)** queue item 13's password-reset link is planned as `avo://reset?token=…`, which the scanner would be equally entitled to open. Needs a call on who owns `avo://` and what the other app uses — `avo-staff://` is the obvious candidate but it is a product/branding decision, and changing a scheme after release breaks any link already in the wild. |
...
| 116 | **I dispatched TWO lane A sessions into the same worktree at once, and only one of them noticed.** Wanting item 5's email driver and item 7's branch report both moving, I sent both to `/Users/koraspond_developer/dev/avo-api`. The second brief opened *“wait for your email-receipt slice to be committed before starting this one”* — **which is not a mechanism.** A freshly spawned agent has no memory of a sibling and starts immediately; I had written an instruction to a session that did not exist yet and treated it as scheduling. The email lane found out the way you find out: at 23:18 `services/reports.ts` appeared as modified in its worktree, a file it had never touched, and the tree went red with five `TS2741`s about a report kind it had never heard of. The hash changed again within 45 seconds and `routes/reports.ts` joined it. **It recovered without being asked and without touching the other session's files** — created a detached `git worktree` in the scratchpad, symlinked `node_modules` and `packages/*/dist` into it, copied only its own files across, and verified there until the other lane committed; then re-baselined on that commit so its own delta stayed attributable. It also reported the interference as a finding rather than absorbing it. | **The whole lane model is one writer per worktree, and I am the only participant who can break that.** A lane cannot collide with itself; it can only be collided with by a dispatcher. `LANES.md` § “Every lane isolates its own resources” is about ports and databases, and it never occurred to me that the thing most easily double-booked is the checkout — because until today I had never dispatched two agents to one column in the same hour. **The near-miss is what makes it worth a row.** Both slices landed green and the tallies reconcile, so nothing in the tree records that this happened; the only reason I know the baseline moved under one lane's feet is that it said so. Had it simply re-measured and reported the larger number, I would have merged two slices believing one of them had added 30 integration specs it did not write. **The rule: a worktree takes one dispatch at a time, and “wait for X” in a brief is a note to a reader, never a lock.** If two slices in one column are both urgent, they are one dispatch with two parts or they are sequential — there is no third option, and a sentence at the top of a brief is not the third option wearing a hat. |
| 115 | **TRUNK RULING: inline emphasis in body copy is 600. Two lanes measured the same question and reached opposite answers, and both measurements were correct.** Lane C served the dashboard and found bare `<b>` computing to **700** — the UA sheet's `font-weight: bolder` against 400 copy — across ~40 emphasised phrases, and set `b, strong { 600 }`. Lane B served the design bundle, measured the same 700, and concluded the opposite: *“the rendered pixel IS 700, that is what the file is”*, so it left the scanner's `Held deposit <b>5.000</b>` at 700 and **flagged the ambiguity rather than resolving it**, naming the exact four places a ruling would touch. **The bundle genuinely contains both readings**, and I checked rather than picked: of 34 `<b>` elements, **two carry an explicit weight and both say 600** — and both are real rendered UI (the activity row's actor). The other 32 carry nothing. Some of those 32 are prose addressed to the developer; **some are unambiguously product copy** — the held-deposit line sits inside a tinted chip with a brand dot, `border-radius:12px`, 12.5px brand-deeper text. So “bare `<b>` only appears in banners” is false, and I nearly ruled on it. **What decides it is `design/tokens/avo-tokens.json`: the designer's own typed system contains NO 700 at all** — `displayXL/L/M/S` 500, `money` 600, `label` 600, `body*` 400. A weight absent from the design system cannot be the intended weight for body emphasis, and the 556-to-11 split puts every 700 in the bundle on a badge. The bare 700s are the browser's default in markup nobody styled. | **The lanes' behaviour is the part worth keeping, and they behaved differently in a way that was right both times.** Lane C had a clean case on its own surface — an explicit `<b style="font-weight:600">` in the same console file — and acted. Lane B had no such anchor in its column (*“nothing in the bundle assigns 600 to that span”*, which is true), so it measured, said the design was internally ambiguous, listed the four places, and **stopped**. That is the correct escalation shape: not a lane refusing to decide, but a lane distinguishing a question its column can answer from one only trunk can. **What I got wrong was the brief.** I told both lanes the design's 700s were badges and “not one is body text” — true of `font-weight:` declarations and false of the rendered document, because `grep font-weight:` cannot see a `<b>`. Both lanes caught it independently and told me so in the same hour. I had counted a census of half the channel and reported it as the whole. **The transferable rule: a census of a declaration is not a census of a rendering.** The design is a document that runs; measuring what it says is not the same as measuring what it does, and the gap between them was 34 elements wide. |
| 114 | **I relayed a lane's statement about its CLIENT as a statement about the WIRE, and it went into another lane's brief as fact.** Lane C's voucher report said `GET /v1/vouchers` serves `{ items, truncated }` — true of the reader it wrote, whose `interface VoucherList` omits the cursor **deliberately**, with a comment saying a “load more” built on an always-null cursor would page for ever. I wrote it into lane D's brief as *“**not** `nextCursor`”*, about the endpoint. `routes/vouchers.ts:262-267` serves **all three keys**, `nextCursor` hardcoded `null`. Lane D caught it by reading the route and printing the raw response rather than trusting the brief, and used `paginated()` — the helper my sentence would have talked it out of. **The same pass produced a defect that is mine outright:** I added the mock's four voucher routes that morning and served the BARE row from `POST` and `DELETE`, where the API answers `{ voucher: … }` and `apps/dashboard/src/api/vouchers.ts` reads `raw.voucher` on both and throws on anything else — so the console could not be developed against the very mock I added to spare it running the real API. Also omitted `nextCursor` from the list. Both fixed and driven; lane D's three `knownBug()`s go red the day they are. | **Two failures, one mechanism, and it is decision 104's.** That row says a cross-lane flag has no expiry and gets relayed as live; this is the same relay one step earlier — not a stale claim but a **re-scoped** one, true of a client and false of a server, and the scope is the half that got lost in my summary. **A lane's report is evidence about what that lane could see.** Lane C never claimed the wire; it described its own parser and said why. The widening happened in my paraphrase, and a paraphrase is where a fact loses its subject. **What makes it worse than an ordinary mistake: I was in the best position of anyone to check.** The route file is in this checkout, the answer is five lines, and I had already read that file twice that day — once to write the brief lane C corrected me on, once to write the mock. **And the mock defect is the same carelessness with the same file:** I read `serialiseVoucher` and reproduced the ROW while skipping the envelope around it, on a route whose only consumer reads the envelope. **The rule, and it is cheap:** when relaying a lane's finding into another lane's brief, quote the lane and name the file it is about — “lane C's reader omits the cursor” survives the trip; “the endpoint does not send one” does not. |
| 113 | **RESOLVED — and my word for it was wrong, which mattered.** I called it a self-deadlock. Lane A took a `pg_stat_activity` snapshot 3s into the hang: **one** connection, `idle in transaction`, its last statement the intent insert. **There is no lock cycle, so Postgres's deadlock detector can NEVER fire** — from the database's side one client has simply stopped talking. It is client-side pool starvation, which is exactly why the symptom (`gateway_unavailable`) names nothing to do with connections. **And the timing argument I built the diagnosis on missed the worse half:** the abandoned insert still LANDS. The timeout rejects the promise but does not cancel the queued query, so it runs the instant rollback frees the connection — **8.057s in, 68ms before the caller is told 502** — leaving a sandbox payment row with `intent_rows = 0`. *"No payment with no intent"* was being violated on **every one of those 502s**, and nothing was watching. The fix gives the sandbox its own pool, shaped by the same `DB_POOL_MODE` knob; threading the caller's `tx` was rejected because it would make the sandbox's payment **atomic with our intent, a property no real processor can offer** — every caller above the seam would then be written against an atomicity that evaporates the day `GATEWAY_DRIVER=myfatoorah` is set. Sibling audit: 35 modules import the module-level `db`, exactly one was reachable from inside an open transaction. `0.047s / 200 / 1 intent / 1 payment` after. **Original finding:** `POST /topups` on the live API answers **`502 gateway_unavailable` after ~10.8s, three for three**. Reproduced locally against `avo_ci` with a control, same code, same database, same request, one variable: **`DB_POOL_MODE=serverless` → 8.052s → 502; the variable unset → 0.045s → 200 with a real `redirectUrl`.** 8.052s is `GATEWAY_TIMEOUT_MS` (8000, `env.ts:191`) — a timeout, not a refusal. The mechanism: `routes/topups.ts:108` runs `createTopUp` inside `sql.begin`; `services/topup.ts:474` calls the gateway from inside that transaction; and `gateway/sandbox.ts:29` imports the **module-level `db`** and inserts on the POOL rather than on the caller's `tx`. `serverless` mode is `{ max: 1 }`, so the outer transaction holds the only connection and the nested insert waits for one that cannot free until the outer transaction commits. **The gateway timeout is the only thing that ends it.** Handed to lane A with the alternatives unchosen — thread the executor into the gateway, move the call outside the transaction, or raise `max` — because each trades a different guarantee and the property to keep (no intent without a payment, no payment without an intent) is not obviously safest under any one of them. | **The correction is the part worth keeping.** "Self-deadlock" is a word that points at the database, and it would have sent the next reader to Postgres's lock tables, where there is nothing to find. **A wrong name for a defect is a wrong search.** What was actually true is duller and more general: a client library with one connection, a caller holding it, and a second call queued behind the first. **This is the exact defect `services/charge.ts` already records — on the other money path.** Its header says `chargeScannerBudget` was hoisted out of `performCharge` because *"a concurrency probe proved that version deadlocks the connection pool"*. Same family, different call site, and the one that shipped is money-IN: the first thing a stakeholder opening this demo would try. **What makes it worth more than a fix is WHY it was invisible.** `max: 10` always has a second connection free, so the bug does not exist locally, does not exist in CI, and does not exist in `e2e/` — it exists only under the one configuration nothing exercises. `db/poolOptions.test.ts` holds both halves of the pool-mode argument and asserts the **options**, never the **consequence**: that a transaction under `max: 1` can still make a nested pool call. That is row 26 wearing a different coat — a spec that pins a number nobody's behaviour depends on, beside a behaviour nothing pins. **And it says something about my own gate discipline.** I have three times this week written that a green check is a claim about a narrower thing than the sentence implies — decision 76 (lint), 89 (typecheck), 101 (int suite), 106 (CI). This is the fifth and the sharpest, because the narrowing is not a missing step: every step ran, and the deployed configuration is simply not among the things any of them run. **A deployment configuration that no gate exercises is an untested build, however green the tree is** — and the honest test of a demo is to use it, which is how this was found and not how it should have been. |
| 112 | **`db:verify` throws away the HOST in `DATABASE_URL` and verifies a LOCAL database of the same name — and the only reason I found out is that the demo database is called `postgres`.** `scripts/db-verify.sh` parses the database NAME out of the URL and then runs `docker exec avo-postgres psql -d "$DB"`. Point it at Supabase and it connects to the container on this laptop. It failed loudly — *relation "salon" does not exist* — **because a local `postgres` database exists and is empty.** Had the demo database been named `avo`, the existence check would have passed, the invariants would have run against the LOCAL `avo`, and it would have printed `ALL INVARIANTS HOLD` about the wrong machine. Settled rather than assumed: the SQL file is host-agnostic, and `psql -q "$DATABASE_URL" -f api/scripts/verify-constraints.sql` against Supabase is **103 / 103, and the database is unchanged by this run** — so the standing claim was true, it was just never produced by the wrapper. `DEPLOY-DEMO.md` now says which command produces it. Flagged to Lane A rather than fixed here: `api/` is its column and it is mid-slice. | **The script's own header is an essay against exactly this.** *"It used to default to the SHARED `avo` database while ignoring DATABASE_URL, so a lane that had exported its own URLs and run this verified somebody else's database and got a plausible answer. Lane B did exactly that. Defaulting is the defect."* The fix took the variable seriously for the database name and **kept ignoring it for the host** — the same defect one level up, in the commit that was fixing it, under a paragraph declaring it fixed. **What makes this the dangerous kind: the wrong answer is green.** A gate that fails on the wrong target teaches you it is broken; a gate that PASSES on the wrong target teaches you the target is sound. And the thing it would have been wrong about is the 103-invariant ledger proof on the provider actually serving the demo. **The transferable rule is not "parse URLs properly".** It is that a check must be able to say WHERE it ran, and `db-verify.sh` already prints `verifying $DB (from $SOURCE)` — a line that names the database and not the host, which is precisely the half that was wrong. A provenance line that omits the ambiguous half is worse than none, because it looks like provenance. |
| 109 | **A manager can now type any price up to 200.000 KD, and `perms.void` is the gate. Is that the permission you want, or does this need its own?** Lane A chose `void` with the argument stated rather than buried: a void is *bounded* — it can only return money already taken, only within fifteen minutes — while a typed price is unbounded and forward-looking, **so granting `void` today silently widens what every existing holder can do tomorrow.** The honest gate is a tenth permission, `perms.customAmount`, which is a four-way break (a `staff_user` column, `StaffPermsSchema` in trunk-owned `packages/types`, the Accounts → Team chips, and lane D's permission census all move together) and so is not a lane's to make. | **The question is about your staff, not the code.** If the people you trust to reverse a charge are exactly the people you trust to invent one, `void` is right and nothing more is needed. If they are different people — a supervisor who may refund but may not price — then the shapes have already diverged and the tenth permission is the only way to say so. Worth knowing while you decide: every custom charge is a marked row carrying a required reason, its own audit action and a 15-minute void window, so the widening is visible rather than silent. |
| 110 | **The 200.000 KD ceiling on a typed price is a number I picked, and it only catches the typo when the real figure is above 20.000.** Eight times the dearest seeded service and twenty times the contract's maximum deposit; nothing a salon legitimately does in one visit exceeds it. **But an 8.000 blow-dry mistyped as 80.000 sails straight through**, and no single ceiling catches the small-charge typo without blocking real work. What actually catches that case is the customer's balance, the near-duplicate guard on a second tap, the 15-minute void and the audit row. | **What I want from you is the real number, per salon.** The right home is beside `salon.deposit_fils` with a Settings control, which is schema + `packages/types` + lane C and wants deciding before it is built rather than after. A salon that never charges above 30.000 should have a ceiling of 30.000 — that is where a ceiling starts catching the mistakes that actually happen. Tell me the figure your pilot salon would set and I will size the control around it. |
| 111 | **A custom charge against a booking is attributed to that artist on the performance report with nothing saying the figure was typed — so a bonus calculated from that report is calculated from a number a staff member chose.** A walk-in custom charge is already safe: no booking means it lands in the named `walkin` bucket, separated from every artist's row. The gap is the held-booking case, where the report shows the artist's name beside a figure that is not her service's menu price. `best-selling-services` has the mirror gap — it counts that charge under the booking's service at the typed amount. | **This is the one place the property “a merchant paying a bonus must be able to tell” is answered only by the audit log, which is not the screen the question gets asked on.** The fix is a `Custom KD` column as a **subset** of `Earned KD`, so the existing sales-vs-artist reconciliation invariant still holds — but it changes the CSV column list that lanes C and D both read, and deserves its own slice rather than being smuggled in. **Not a blocker for the demo** (the seeded data has no custom charges in it) and it is a blocker for the first salon that pays commission. |
| 108 | **I ran the two api gates in the order a person would and got a FALSE RED on the one invariant that guards the ledger.** After merging lane A I ran typecheck, unit, `test:int` and `db:verify` against one database (`avo_ci`), in that order. `db:verify` reported **invariant 5 FAILED** — `member.balance_fils = sum(member_wallet entries)`, *`AP-M-mu1e2aa060-1 off by 59000 fils`* and more — on a tree whose 378 integration tests had just passed. **The rows are the integration suite's own fixtures.** They mint a member with a balance directly rather than through a top-up, so no `member_wallet` entries back it; every one of them is an `AP-M-…` id no product code ever writes. Re-run `lane-db.sh ci` and `db:verify` alone on the fresh database: **103 / 103, and the database is unchanged by this run.** CI never sees this — `ci.yml` gives `db:verify` its own `avo_migrate_check` and the int suite its own `avo_int_check` (lines 113 and 189), so the separation that makes both honest **already exists and is written down nowhere a person running the gates by hand would find it.** `api/README.md` is the map of the test surfaces and it is the place; noted there. | **The value here is not the fifteen minutes; it is which invariant it was.** Number 5 is the ledger reconciliation — the single assertion standing between this product and a balance that disagrees with its own history — and I had it in front of me reading FAIL, immediately after merging a change that adds a new way to move money. **The first thought available was that lane A had broken the ledger, and that thought was wrong.** Decision 103's rule is *a lane's red gate is a claim about that lane's base*; this is the same rule pointed at myself — a red gate is a claim about **the database it ran against**, and the check is one command. What makes it worth a row rather than a shrug: this red is **reproducible on demand and looks exactly like the real thing**, so the next person to meet it either loses the afternoon lane A nearly lost, or — far worse — learns that invariant 5 flakes. A gate that cries wolf about the ledger is a gate somebody will eventually wave through. The `int`-then-`verify` order is the NATURAL one to run by hand, which is why leaving it undocumented was the actual defect. |
| 107 | **Trunk change: `customAmount` joins `TransactionSchema`; the reason string deliberately does NOT.** Lane A's custom-amount slice serves `customAmount` and `note` on `GET /charges`, and neither is in trunk-owned `packages/types` — so `e2e/support/contract-drift.ts` would have reported both as **stripped**, which is drift (7) exactly: *zod does not fail on a field it does not declare, it DELETES it*, and the last time that happened it removed the entire 15-minute reversal feature between the server and the screen. `customAmount` is now required on every transaction, for the reason `voidedAt` is: a charge whose price a staff member invented is precisely the charge a customer has cause to query, and `false` is a positive statement rather than an absence she has to interpret. **`note` is the half that needed a decision rather than a patch.** `transaction.note` is a general-purpose internal column — void reasons, *“Cancelled by the customer”*, *“Deposit larger than the visit”*, *“No-show · deposit returned automatically”* and an owner's free-text adjustment reason all live in it. Declaring it on the customer's transaction type would put every one of those **one careless serialiser away from her own activity list**, and today it is kept off her wire by a single ternary (`note: t.customAmount ? t.note : null`) — a rule in one place with nothing stopping the second. So it stays a MERCHANT-ROUTE key beside `feeFils`, whose column already carries the words *“merchant-visible, customer-never”*, and `serialiseTransactionForMerchant` now returns it masked the same way. Lane D annotates it `wireOnly` **with that reason**; the annotation is checked in both directions, so it cannot become where the drift goes to be forgotten. | **tsc named every place this was going to hurt, and there were four.** `serialiseTransactionForCustomer` is meant to be the one builder, and `charge.ts` (twice), `booking.ts` and `order.ts` each hand-assemble the row they feed it — the same *“one payload built in three places”* `contract.test.ts` recorded when `voidedAt` was added, still unfixed, now four. I filled them rather than collapsing them: collapsing is the right change and it is not a rider on a type addition. **Two recorded wire fixtures went red, and both were right to.** `apps/scanner/src/api/charges.test.ts`'s `WIRE` constant and `apps/wallet/src/api/shop.test.ts`'s `ORDER_201` are recorded responses — the thing `contract-drift.ts` opens by warning about, *a fixture drifts alongside the schema it is meant to police*. The wallet one had predicted this in writing: *“if the schema ever grows a required thirteenth, this fails here rather than on every real response in production.”* It grew one; the spec caught it; that is the whole return on having written it. **What I did not do:** flip a mock fixture to `customAmount: true` to give the scanner something to render. That would silently change what an existing row means to every spec asserting on it, and appending a sixth changes a length three surfaces count — so `packages/mock` accepts a real `amountFils` charge instead, including the 403, the ceiling, the fraction refusal and the required reason, because **a scanner built against a mock that forgives an unauthorised typed price would never see the refusal it has to draw.** |
| 106 | **The guard I added to end silent green could not fire, and it turned every CI run red for a reason that had nothing to do with the code. Six red builds in one day, and I reported green all day without looking at one of them.** Decision 101's step greps the vitest summary with `^ +(Tests|Test Files)`. vitest KEEPS COLOUR ON IN CI, so the line arrives as `\e[2m      Tests \e[22m ...` — an escape sequence between the start of the line and the word. Neither grep can match, and the two consequences run in OPPOSITE DIRECTIONS: the closing `grep -E '^ +Tests '` failed every run, so the step exited 1 **on a tree whose 360 integration tests all passed**; while the skip guard — the entire point of 101 — was **silently unable to fire at all**, which is the exact state it was written to end. Fixed by stripping ANSI into the file the greps read; `tee` still shows colour in the console. Proved in both directions against captured CI bytes: before, the skip guard is blind and the final grep fails a clean run; after, it fires on a skipped run, does not fire on a clean one, and the final grep matches. | **IT PASSED LOCALLY BECAUSE VITEST DROPS COLOUR FOR A NON-TTY PIPE. I verified the guard in the one environment where the bug is invisible** — decision 101's own row says *"the guard on it was wrong the first time and testing is how I know"*, and it was wrong a second time, in a way that testing where it runs would have caught and testing where I stood could not. **The sharper failure is mine and is not the regex.** Six pushes to `dev` today, every one red, and I did not look at CI once — I reported green from local runs and called that the gate. `pnpm check` on my machine and the pipeline are different claims, and I used the word for both. That is the same shape as the whole week's findings arriving from one direction: a true statement about a narrower thing than the sentence implied. **What makes it worse than a broken assertion: a gate that fails for a spurious reason teaches people to ignore it.** Two of the runs it was buried under were `main`'s only CI history since 2026-08-24, so the tree looked chronically red and the signal was worth nothing. **The rule this leaves:** a gate must be exercised in the environment that runs it, and a CI change is not verified until a CI run has gone green — not until it passes locally. Check the run, every time, before saying the word. |
| 105 | **The route scanner behind the permission census invented routes, and the fix for that would have introduced the module's only genuinely silent shape.** `e2e/support/perm-census.ts` matched a registration's type arguments with a lazy, newline-crossing `(?:<[\s\S]*?>)?`. Verified at trunk before dispatching, on the live regex, with two synthetic sources differing only in one path being lifted into a `const`: inline gave `DELETE /v1/salons/:id/services/:oid/image` + `GET /v1/images/:imageId`; the const gave **`DELETE /v1/images/:imageId` alone** — one registration's METHOD welded to the next one's PATH, a route nobody wrote, **and both real routes gone**. Latent only because every path in `api/src/routes/` happens to be an inline literal, so the trigger is an ordinary refactor. **The fix had been sitting uncommitted in the shared qa worktree for TWELVE DAYS**, skipped by four lane D commits that each staged their own paths correctly. **Three things were wrong with it, and the third is the finding.** `registrationsIn` was not exported, so the bug could not be pinned at all. Its comment claimed the ghost fires `contract.test.ts`'s check, which did not read this module. And `skipTypeArgs` **counted the `>` of an arrow type** — `app.get<{ Reply: { render: () => string } }>('/p', h)` closed its type-argument list at the `=>`, so the next character was not `(`, and **both passes dropped the call**: `callSites` reads "not followed by `(`" as "not a call at all". Not censused, not reported by the ambiguity pass, no route, no alarm. **So the walked reader would have been NARROWER than the regex it replaced, and the trade would have been a loud wrong answer for a silent one.** Guarded, and the nested case with it. Route set byte-identical across the whole change: 144 routes, 103 gated, 56 ungated, 159 ledger lines, 0 ambiguous. | **Why this matters more than a miscount: this scanner is what asserts non-negotiable #7.** A route it loses is a route whose permission gate stops being generated as a probe, and a probe that is not generated cannot fail — decision 90's shape, where `'either'` turned out to be asserted by nothing. **The chain was checked rather than assumed, by replaying the census's own set arithmetic over a ghosted ledger**, and it holds: the probe is dropped silently, but the ALARM is loud, names both lost routes, and already lists *"a path moved into a const"* as a reason — and it holds only because that spec has no exemption list. **The one spec that lies is the third one.** It reports the invented route as unpinned and invites the reader to add it to `PINNED_COVERAGE`. So the failure mode is not a confusing message; it is **a correct-looking instruction to pin a route nobody wrote**, which would then be asserted forever. A remediation message is part of a gate's contract, and this one is wrong in the one state it exists to describe. **Two things worth carrying forward.** First, `e2e` had a THIRD definition of "what is a route registration" (`support/contract-drift.ts`'s `discoverGetRoutes`, anchored on the literal receiver `app.get`, single quotes only, const-blind, with no ambiguity pass at all — its only floor is `>= 25` against 55 real GETs). Both readers returned the same 55 paths, so nothing was wrong; **collapsed the same night** — `getRoutesIn` is now `registrationsIn` over `stripComments`, one reader for both censuses. **Three readers of one fact is the defect, not the disagreement they have not had yet** — and the collapse proved that exactly, because the difference nobody had decided was not any of the three weaknesses on the brief. It was **comment stripping**: `censusOfRoutes` stripped comments and `discoverGetRoutes` did not, so a **commented-out `app.get('/…')` counted as a served GET** and would have demanded a probe, or an `UNMODELLED` reason, for an endpoint nobody serves. Never decided, only never exercised. The `>= 25` floor was replaced rather than raised: against 55 real GETs it was thirty routes of slack and could not detect the collapse it nominally guarded. And the lane applied #104 unprompted — `perm-census.ts`'s now-false sentence about which spec the ghost fires was **kept as a quote rather than deleted**, on the grounds that the paragraph is already a record of a comment naming the wrong spec and rewriting it silently would erase the second instance of the same failure. Second, lane A and lane D fixed opposite halves of the same closure-order defect in parallel, from bases a merge apart, without either knowing — lane A made the API's order deterministic (RETURNING cannot be ordered in Postgres, so one sort call), lane D made the spec compare SETS since no design file specifies a sequence. Both are right and they compose, but the duplication is decision 103's cost showing up as wasted work rather than as a phantom. |
| 104 | **A comment that flags another lane's defect becomes a false claim the moment that lane fixes it, and nothing expires it — so it gets relayed back as live.** `api/src/routes/orders.ts` carried a paragraph headed *"ONE CONSEQUENCE IS OUTSIDE THIS LANE AND IS FLAGGED RATHER THAN FIXED"*, saying `ShopOrders.tsx` prints "Collecting at the salon" over an erased delivery and that the fix is lane C's. It was true when written. **Lane C landed the fix — the third arm is keyed on `fulfilment` and renders "Address no longer held" — and the comment kept saying "flagged rather than fixed" and "reported in the lane handoff".** Lane A then read that comment during the next slice and reported the defect to trunk as **still standing**, in a handoff whose other half was *fixing this exact class* in `db/schema/shopOrder.ts` (a header arguing "WHY THERE IS NO `shop_order` TABLE" in the file next to the one that creates it). **One report, one instance fixed and one instance propagated.** Verified before correcting: `ShopOrders.tsx:604,611` render both arms, and the api-side paragraph was the only thing still claiming otherwise. Corrected, and dated, and it now ends by telling the reader it cannot know — the only honest thing a comment can say about another file. | **This is the mechanism behind the memory note "stale not-built comments", stated properly for the first time.** The failure is not carelessness; it is that a cross-lane flag has **no expiry**. Everything else in this project that asserts a cross-lane fact expires by construction: a `knownBug()` spec goes RED saying *"This bug appears to be FIXED"*, `AWAITING_MERGE` fails its own arrival check the hour a route lands, `NEEDS_PLATFORM_CREDENTIAL` emptied itself the day the harness could mint a platform principal, and `wireOnly` is checked in both directions so it cannot become where drift goes to be forgotten. **A comment is the one artefact in the tree with no such mechanism**, and it is the artefact lanes use for exactly this purpose, because it is the only one that costs nothing to write. **The rule, for lanes and for me: a cross-lane defect goes in a spec that expires, not a comment that cannot.** A lane that cannot write into the other lane's column can still write a `knownBug()` in `e2e/` — lane D's column is `**/*.test.ts`, so any lane can hand it one — and that assertion goes red the day the other lane fixes it, which is the day the note becomes a lie. Where a comment is genuinely the right place, it must name what it cannot know and point at the file that can. **And the sharper half is about me, not the lanes:** I relayed it. Decision 103 was a lane reading a stale BASE; this is a lane reading a stale COMMENT; both arrived as confident, specific, file-and-line reports, and both were true once. **A dispatched lane's report is evidence about what the lane could see, not about the tree** — the check is the same either way and it takes one grep. |
| 103 | **A lane reported the trunk gate red, and the trunk gate is green — the red was its own base, one integration behind.** Lane A closed decision 89 and flagged, correctly and outside its column rather than editing it: `pnpm check` fails on `feat/api` at `e2e/contract.test.ts` — *`STRIPPED $.items[].memberErased`, the wire sent false and the parse deleted it* — with the diagnosis that `memberErased` is served by `api/src/http/serialise.ts` and exists nowhere in `packages/types`, so the fix is a trunk types operation. **Checked before doing any of it, and every part of the diagnosis is true while the conclusion is wrong.** Decision 100 was closed by FOUR commits in four lanes. `feat/api` had merged `dev` at `0820864`, which contains `ee756ee` — lane A's own API half, the half that starts serving the field — and **not** `ec20229`, lane D's half, which is the `wireOnly` annotation that explains it. Verified with `git merge-base --is-ancestor` in both directions. So the lane was looking at a tree where the server had learned to send a key and the contract file had not yet learned to expect it: **a state that existed on no branch anyone would merge, and that only a lane's own base can produce.** On `dev` with lane A merged, `contract.test.ts` is **110 passed (110)**, both `GET /v1/salons/:id/orders` probes included. **And the proposed fix would have damaged the contract it was trying to satisfy.** Adding `memberErased` to `ShopOrderSchema` is the exact move that annotation argues against at length: an order does not have a name, does not have a phone, and **is not erased** — the member is, and the row is a join across two lifetimes. An optional field there cannot tell *"erased, deliberately no number"* from *"the board forgot to send it"*, which is the ambiguity `countedPage` exists to remove and the one a client's defensive default turns back into a blank `tel:`. So a red gate reported from a lane would have bought a schema change that reopened #100's design question. | **Mine, not the lane's.** I dispatched onto a base I had not levelled after the previous integration, and the four-lane merge that closed #100 is precisely the shape that makes a stale base incoherent rather than merely old: each lane's half is inert or wrong without the other three. The lane did everything right — it stayed in its column, it named the file and the commit, it said explicitly that its own change could not reach the wire, and it reported 21 of 22 turbo tasks green so the failure was locatable in one read. **What this costs if not caught is worse than a wasted hour**, because the report is credible: it names a real served field, a real absence in `packages/types`, and a real failing assertion, and the only thing wrong with it is a fact no lane can see from inside its worktree. **The rule, and it belongs to the dispatcher:** a lane's red gate is a claim about *that lane's base*, and it is not evidence about trunk until the same command has run on trunk. Level every lane's base at every integration — `RUNBOOK.md` § the integration hour already ends with "push and level every lane", and skipping it is what produced this. And when a lane proposes a trunk operation, read the contract's own argument before performing it: this one had the answer written into the annotation the lane could not see. |
| 102 | **A FOURTH door onto the same tombstone, found by the inventory that closed the first three, and deliberately left open.** `GET /members/:id` — the scanner's manual-lookup RESOLVE — answers with `counterEnvelope(db, serialiseMember(m), …)`, and `serialiseMember` (`routes/auth.ts:1534`) carries the **full phone**. Proven live: `{"member":{"id":"VF-DEAD","name":"Deleted account","phone":"+990818102096528","balanceFils":0,…}}`, reachable in two clicks from `GET /members?q=Deleted` (the search matches the tombstone NAME, and its digits branch matches the synthetic number). **And the phone is the smaller half.** `resolveMember` (`services/memberSearch.ts:436`) has no `erasedAt` predicate, and `charge.ts`, `charges.ts` and `counter.ts` contain no erased check at all — while `vouchers.ts:211,422`, `adjustments.ts:165` and `accountResets.ts:137` each refuse an erased member **by name** with `member_erased`. So the manual path can carry an erased account onto the charge screen. The QR path cannot: erasure deletes `walletToken`. | **Why this is queued rather than fixed with the other three.** It is not the same shape. The endpoint is explicitly UNMODELLED in the contract census, but it embeds `MemberView`, and `MemberSchema.phone` is a non-nullable `PhoneSchema` in **trunk-owned `packages/types`** — so nulling it is the four-way rebase row 28 describes, not a lane fix. The alternative is to refuse the resolve outright, which matches the established `member_erased` pattern in the three sibling routes and is arguably the *more* correct answer — but it changes merchant behaviour, and there is a real counter-argument that a merchant needs to confirm "yes, that account is gone" rather than meet an error. **Which of those two is right is a product call, and guessing it would be inventing a feature.** **Why it has stayed invisible:** erasure defers while `balance > 0`, so an erased member's balance is 0 and a charge dies on insufficient funds — an accident of the deferral, not a guard, and it shows the staff member the wrong sentence. **`dev` is not left red for it** (rule 1): lane D's probe is an `it.fails()` naming this row, so the defect stays asserted and inverts the day somebody closes it. **The method is the transferable part.** Row 100 ended "what nobody had was an inventory of everything that *reads* a member after she is gone." Building that inventory is what turned one reported door into four, and lane D's suite now derives it off disk — every `member.phone` projection in `api/src` must be accounted for — so door number five announces itself instead of waiting to be noticed one cell over. |
| 101 | **RESOLVED 2026-09-10 — and the cost was fifteen seconds.** The step is in `.github/workflows/ci.yml`, verified by running its exact commands locally: **21 files, 346 tests, all passing, in 14.9s.** So there was never a time argument for the omission; nobody had wired it. Its own database (`avo_int_check`) rather than sharing `avo_migrate_check`, because the verify step runs 93 invariants against that one and several int fixtures leave rows by design — sharing would make the step order load-bearing and undocumented. Connected as **`avo_app`, not the owner**, which is the whole point of these suites: *"a limiter proved under the owner role is a limiter proved against privileges production does not have"*. **The guard on it was wrong the first time and testing is how I know.** I wrote "fail if fewer than one spec passes" — useless, because with the variable unset **fourteen still pass** (they need no database) while 332 skip, so a floor of 1 never fires and a floor of 346 goes stale next slice, which is row 26's defect. What actually separates the two states is that a correctly configured run **skips nothing**: `346 passed (346)` against `14 passed | 332 skipped (346)`. So the guard asserts a property rather than a count, holds whatever the total grows to, and fails in both directions. Both directions verified. **Twenty-one integration suites had never run in CI — including every proof that the money paths are correct.** Verified on trunk: `api/vitest.config.ts:97` excludes `**/*.int.test.ts` (deliberately, and its own comment says why — an int spec is not a unit spec), `pnpm check` is `turbo run typecheck lint test` which calls `test` rather than `test:int`, **CI never invokes `test:int`, and nothing anywhere sets `AVO_INT_DATABASE_URL`** — zero matches in the workflow. Twenty-one files. Every one of them green by never running. | **The third and worst member of this family.** Decision 76: `pnpm check` reports a lint pass it never ran. Decision 89: the typecheck does not read the specs. This one: the gate does not run the suites that prove the charge is atomic, the token is single-use, the deposit applies once, the voucher claims once, and the address is erased. Those suites are where this project's hardest work lives — lane A's `erasureAddress.int.test.ts` is deeper than lane D's own e2e coverage on the money half, and it is `describe.skip` and green in the pipeline. **What makes it worse rather than better: it is nearly free to fix.** CI already boots the compose Postgres, already mints a throwaway database, and already runs `db:verify` against it — so a `test:int` step needs a database it has and one environment variable it does not set. The exclusion itself is correct and should stay; what is missing is the second step that runs what the first deliberately skipped. **Also recorded from the same slice:** lane D removed two stale counts rather than renumbering them, on row 26's ruling that a number nothing checks is itself the defect — this one had been wrong twice — and replaced them with a spec asserting the thing that actually matters, that the erasure job's census still names both address-bearing tables. |
| 100 | **RESOLVED 2026-09-10 in `f6982ed` (lane C), `e4f4c81` (lane B), `ee756ee` (lane A) and `ec20229` (lane D) — and it was THREE doors, not one.** The report named the fulfilment board. An inventory of every `member.name`/`member.phone` projection in `api/src` — the thing this row said nobody had — found the same live join on `GET /salons/:id/bookings` and on `GET /artists/me/bookings`, and **the third is worse than the one that was reported**: `apps/scanner/src/screens/BookingsScreen.tsx` turns that phone into a `tel:` **and** a `https://wa.me/<digits>` button, so the scanner does not merely display a fabricated number, it offers to message it. Narrow to reach (erasure defers on `deposit_held`, so it needs a `completed` booking inside the route's 24h window) but `erasure.ts` keeps `booking` rows deliberately, so the row is there. **The contract, decided at trunk and applied identically to all three:** `memberErased: boolean` derived from `erased_at` **and** `memberPhone: null`, both, with `memberName` left as the tombstone. Neither half alone is enough — a boolean alone still ships a tombstone in a field the wire calls a phone, and rendering those digits as plain text is barely better than linking them because a merchant can copy them into a handset; a null alone is indistinguishable from *no phone on file* and tells a client nothing about why. **The live-delivery objection resolves in favour of nulling:** `erasure.ts` scrubs the address snapshot on *every* delivery order regardless of status, so a `preparing` order for an erased member is already unfulfillable and the number reached nobody either way. Lane A put the derivation in one place (`http/serialise.ts § serialiseMemberContact`), off `erased_at` and never off a `+990` prefix match — a prefix match in the server is the same rotting coupling the field exists to spare clients, moved one process left. **Original finding:** after erasure the merchant's order board still offers a working `tel:` link to the tombstone number. `erasure.ts` mints `+990…` as the member's phone, and the fulfilment board's live `member` join serves it as an ordinary phone — so the row reads "Deleted account" beside a **dialable** number that looks like a real contact and reaches nobody. Found by lane C while fixing the address sentence one cell over. | **The address scrub is complete and this is not a hole in it** — the number is a deliberate tombstone, not residue. The defect is that a tombstone is being *presented as a contact*: a merchant chasing a delivery taps it, hears nothing, and learns nothing about why. **It needs an API signal, not a client guess.** The only client-side detection available is string-matching a server constant, which is exactly the shape that rots — the prefix changes and the UI silently starts offering the link again. So either the payload says the member is erased, or it does not serve a phone at all for an erased member. Lane A's column. **Worth noting the pattern:** this is the third consequence of erasure discovered *downstream* of the erasure fix rather than in it — the false "Collecting at the salon" sentence, two of lane D's own specs going stale, and now this. The scrub was correct; what nobody had was an inventory of everything that *reads* a member after she is gone. |
| 99 | **A pickup order at a two-branch salon records an assumed branch and nothing tells the customer where to collect — and the fix must not let her choose the branch her money is filed against.** Lane B stopped rather than faking it: `POST /orders` takes no pickup branch, refuses a client-supplied `branchId` by name, and `services/order.ts` resolves the branch itself, writing `branch_assumed = true` at a multi-branch salon. Amara has two open branches. So the tile reads *"Pick it up at the salon when it is ready"* and the order row *"Collect at the salon"* — **the salon, never a branch**. | **It rejected three fakes and named the dangerous one:** show a picker and drop the value, so she believes she chose Salmiya, the row says assumed, and nothing on screen ever said the location was unsettled. That is the branch-picker slice's "show them under every branch" lie wearing a costume where the cost is a customer standing in the wrong salon. **Trunk's ruling, which the contract change must respect:** a pickup branch is a **fulfilment fact** and belongs on `shop_order`. It must **not** become `transaction.branch_id`. The distinction is non-negotiable #2: where she collects a bag is her choice, but the branch a charge is filed against decides which boost multiplier applies — and a customer-asserted branch that reached `transaction.branch_id` would be a client choosing its own multiplier, which is exactly what item 4 built device enrolment to prevent. So: a customer may assert *where she collects*; she may never assert *where the money happened*. Those are two columns, and the whole safety of the feature is that they stay two. **Also found by lane B, and lane A's to fix:** a **delivery** order still queues a **pickup** receipt (`queueReceipts(… pickup: true)` unconditional, confirmed in SQL on a real delivery order) and its audit detail ends *"· to collect"*. So a customer who chose delivery is sent a receipt telling her to collect it, and the merchant's log records "to collect" for something going to a driver. Both were written when the shop was collection-only and were not revisited when `fulfilment` landed. |
| 98 | **The `support` platform-role preset holds `accounts: true` with `audit: false` — the power to mint wallet credit without the power to read the log that records it.** `PLATFORM_ROLE_PRESETS` at `db/schema/platformAdmin.ts:166-169`. `analyst` is correctly `false`. **Latent**, asserted in SQL against a live database rather than read off the seed: `owner/t`, `analyst/f`, `admin/t`, and **no `support` row exists at all** — one `POST /v1/platform/admins {role:'support'}` makes it live, because the preset is applied at invite and re-applied on every role change. | **Found because trunk's brief was wrong by one feature, and the correction is what matters.** Trunk briefed that a voucher is the first object letting AVO put money into a salon's wallet liability without the salon acting. It is not: `POST /members/:id/adjustments` already did, on this same `accounts` gate, with `walletAdjustedPosting` crediting `member_wallet` exactly as `voucherRedeemedPosting` does. **Item 10 added a second *instrument* on an existing authority**, so the exposure dates to whenever adjustments and the `support` preset first coexisted — and **nobody closes it by reverting vouchers.** What *is* new is that a voucher is **deferred**: it materialises on a `requireMember` endpoint at a moment the issuer does not choose, and the void stops working the instant she redeems it. So the two instruments differ in when the money moves, not in who may cause it. **`accounts` doubled with item 10** — three routes to six, two of which create money. **Whether support should mint money at all is Aftab's call, not a test's**, and presets are lane A's column, so this is reported rather than changed. The pairing is the part to decide on: `accounts` without `audit` is not a smaller permission than both, it is the same power with the record removed. |
| 97 | **RESOLVED 2026-09-10 in `1f6bfb3` — and "the address book is a plain `DELETE`" was wrong, in a way that would have failed silently.** `shop_order.address_id` references `member_address` with `ON DELETE restrict`, which is why the addresses route soft-deletes, so **the book cannot be deleted while an order points into it**. The snapshot is nulled first and the book second, in one transaction. A plain `DELETE` would have failed for exactly the addresses that had been *used*, and failed into `result.failed` per member — silently. **Erasure did not reach a customer's address, in either of the two places it lives — and the privacy policy still does not mention collecting one.** `member_address` and `shop_order` appear in exactly four files in `api/src` (schema, index, route, order service) and **neither appears in `services/erasure.ts`**, in code or in its table-by-table header (which trunk called "twelve-table"; the census is now 15 rows, and was 13 then — a count in this file going stale again). Not deleted, not kept on an argument, not deferred, not flagged — absent the way a table nobody has thought about is absent. Lane D proved it behaviourally with two `knownBug` specs: after the erasure job runs, **her address book survives intact and the order snapshot survives with it**, served to `perms.shop` beside a member row now reading "Deleted account". | **Item 7 introduced the first personal data in this product with a street in it, and the erasure path was written before that data existed.** The job's own justification does not extend to it, it **inverts**: *"a booking pointing at a tombstone identifies nobody"* is true of a visit count and false of a street. **De-identifying the name while keeping the address is the worst of the two available outcomes** — the record still locates a person and no longer says who, so it is neither erased nor usable. **Two separable fixes, both lane A's:** the address book looks like a plain `DELETE`; the snapshot needs a migration, because `shop_order_delivery_has_an_address` refuses the null-out. And **the third part is not engineering's at all: the privacy policy's § 2 collection notice does not mention an address**, and that document is with counsel now — so the disclosure and the retention horizon are decisions to make before launch, not after. Separately confirmed *correct* and documented: a soft-deleted address stays readable through a past order, deliberately, so a delivered order remains answerable. The gap is that `DELETE /members/me/addresses/:id` is the customer's only deletion and reaches **one of two copies**, with no horizon on the other. |
...
| 89 | **RESOLVED 2026-09-10, and the scope in this row's own "not fixed" argument was wrong.** It said including specs "will surface an unknown number of existing drifts across every package at once". Measured before anything changed, per decision 75: **only three of nine packages excluded their specs at all**, and two of those cost **nothing** — `packages/types` and `packages/tokens` typechecked their specs with **zero errors** and landed as a trunk operation. `api/` was the sole real holdout, at **41 errors**, and of those **two were drifts and 39 were strictness**. Nothing in the workspace excludes a spec from typecheck now. **The two drifts, and neither was the fixture this row was opened about.** `routes/salons.test.ts:27`'s `SalonRow` had no `emailEnabled` — the NOT NULL column *I* landed for item 11 (row 88), whose own fixture drift was caught by nothing. Latent, not broken: no assertion reads it. But the handler's receipt-channel floor is `patch.emailEnabled ?? before.emailEnabled`, so the first spec written against this fixture for `receipt_channels_required` would have read `undefined` off the current row and **refused a salon whose email is on**. `routes/support.test.ts:86` asserted `role: 'founder'`, **a value not in the union** (`owner | admin | analyst | support`) — and **it was asserting nothing**: `queueScope` branches on `principal.kind` and reads no role at all. Worse than inert: `role: 'founder'` beside `owner: true` is a pair the CHECK `platform_admin_owner_flag_matches_role` refuses to store, so the fixture described **a console admin the database cannot contain**. `founder` is the stale word at `design/api-contract.md:616`; lane C wrote it at its own boundary parse and corrected it twice. **This was the third copy and the only survivor — because it was in a spec.** **Not a no-op, proved four ways**, including the negative control that matters: with the old `exclude` restored, the `founder` plant produces **zero errors**. Gate cost `api` **2.76s → 3.12s** (+0.36s, median of five). No `as`, no `!`, no `@ts-expect-error`; one pre-existing `as string` removed. | **A cousin of decision 76 and arguably worse.** 76 is `pnpm check` reporting a lint pass it never ran; this was `pnpm check` reporting a typecheck that did not cover the files most likely to lie. A test fixture is a *claim about a shape* — exactly the artefact whose drift a compiler should catch, and the one place the compiler was told not to look. It is also the mechanism behind decision 78: a fixture with the right shape and stale data is invisible to a typecheck, and here the shape was not even checked. Originally found when `routes/support.test.ts`'s `MERCHANT` fixture turned out to be typed `StaffPrincipal` and **missing `deviceId` since that field was added**, with lane A's own change about to extend the lie to a second field. **The lesson is the correction, not the fix.** Deferring this was justified by an estimate of its blast radius, and the estimate was three times too large in packages and unbounded where the truth was 41. Two of the three packages were free; the measurement took minutes and the argument for waiting did not survive it. **What the exclude was actually costing was not 41 errors, it was the two claims nobody could see** — and one of them was mine, three hours old, in the feature I had just called done. Its remaining family member is 76, which is genuinely a scheduling call: no linter exists, and adding one now lights up five finished surfaces at once. |
...
| 82 | **RESOLVED ON THE SERVER 2026-09-11 — and this row's own claim was proved only half through, which is the part worth reading.** The enrolment landed (`device_enrolment`, migration 0043): `charge.ts:496` passes `ctx.principal.enrolledBranchId` into `resolveBranch` as `supplied`, which verifies it against the salon and returns `established: true`, so `branch.established ? branchId : null` hands the real branch to `loadPromotionInputs` instead of a null. **The sentence below saying "there is no server-side device→branch record anywhere" is stale and kept only as the record of what was true when this was written.** `devices.int.test.ts` proves the BOOST half — 17/17 against real Postgres, including the control that makes it evidence rather than a happy path: **the same till re-pointed from BR-KWC to BR-SAL earns 2x then 1x**. **But this row's own text says a null branch "resolves no boost AND NO HAPPY HOUR", and the proof cited for the fix covered the boost only** — `branch_assumed = false` and `visits +2`. Half the sentence was asserted; the other half was inferred from the two lookups sharing a line. Closed 2026-09-11 in `f2bbb6a` with four arms: the window scoped to the enrolled branch pays; **the window scoped to the OTHER branch, same enrolment, does not** (the discrimination — a server applying every window to everyone passes arm 1 and fails this); revoked goes back to assumed; and a `branchId: 'all'` window pays in both states, which is the arm a narrowing of the branch fix would take out. **The detail that makes arm 1 mean anything: the enrolment is BR-SAL, not BR-KWC.** `x2visit` is the only reward that moves visits and BR-KWC's boost is already `visit = 2`, so a window proved there reads +2 whether it applied or not — borrowing the boost group's own number. Five mutations, each red on exactly its own arms; the sharpest removes `transaction.promotion_id` from `charge.ts` and reddens **the row only** — visit counts stay green while the receipt loses its ability to explain itself. **A latent flake was found in the group above while doing it:** `HH-01` ships ON for Sun/Mon/Tue 16:00–18:00 across all branches, so for six hours a week it would have made the existing "unenrolled: assumed, and no boost" spec read +2 and go red for a reason unrelated to enrolment — never triggered because nobody had run the suite in that window. **What is NOT closed is surface coverage rather than server behaviour.** All three parts of item 4 exist — enrolment (A), `apps/scanner/src/screens/EnrolScreen.tsx` (B), `apps/dashboard/src/routes/Tills.tsx` (C) — and the server half is proved end to end, but the two screens a merchant actually reaches it through are not covered by this proof. **And one artefact still says otherwise:** the `knownBug` in `e2e/promotions.test.ts` charges from an unenrolled scanner session, so it can never go red on this case — a permanent green wearing the costume of a pending fix, which is decisions 104 and 105's class. Lane D is retargeting or retiring it. **ORIGINAL FINDING, kept verbatim below because the resolution above is only legible against it.** **The moment a salon adds a second branch, every per-branch earning rule silently stops being applied.** Verified chain: `branch.ts:128` returns `established: rows.length === 1`; `charge.ts` passes `branch.established ? branchId : null` into `loadPromotionInputs`; a `null` branch resolves no boost and no happy hour. So a one-branch salon's boosts pay, and **a two-branch salon's boosts are stored, served to both clients, and applied by neither** — the charge handler's own comment says exactly that. Every row they could have touched carries `branch_assumed = true`. This is also why item 3's staff report and the Overview branch filter both carry assumption caveats: they are downstream of the same gap. | **RESOLVED 2026-09-09 in `45a60a1`, and the proof is one line: an enrolled till at a two-branch salon charged with `branch_assumed = false` and `visits +2`. That had never happened in this product.** **Adding a branch was therefore not a configuration task, it was a regression** — a merchant who opens a second location loses the earning rates she is still being shown and still editing, with nothing telling her. That reframes what Aftab asked for: the request was "how to set up branch-specific stuff for a new branch", and the answer is that the per-branch machinery exists end to end EXCEPT for the one link that establishes where a charge happened. **The fix is named in the code and half-built.** `POST /charges` deliberately has no branch in its body — a client naming its own branch is a client choosing its own multiplier, non-negotiable #2 with extra steps — and `StaffPrincipal` carries branch ACCESS rather than a current location. `services/branch.ts` already accepts a `supplied` branch from "an enrolled, branch-bound device" and returns `established: true` for it. **What is missing is the enrolment: `session.device_id` is a nullable string the client sends, and there is no server-side device→branch record anywhere.** So item 4 is: a device enrolment that binds a scanner to a branch (lane A), the scanner's "Set up this device" flow choosing it (lane B), and the dashboard managing it (lane C) — in that order, because the others consume the contract. A default boost row for a new branch was considered and rejected as a separate matter: a default earning rate is a commercial commitment, and writing one because the shape was inconvenient is decision 80's mistake again. |
