...
| 65 | **An 8-test flake on `POST /charges` → 500 under cold-cache concurrent load, seen once and not reproduced.** Lane A's first `pnpm check` (cold turbo cache, 21 concurrent tasks, 9m42s) failed 8 e2e specs with a 500 on the charge path. It chased it rather than shrugging: the full e2e suite standalone is 1174/1174 at baseline **and** with its change, and a `pnpm check` re-run is green both ways. Not attributable to that diff. | Recorded because **a 500 on the charge path is the one flake in this product nobody should file and forget.** `POST /charges` is the atomic money transaction — token consumption, debit, visit increment, tier evaluation, receipt queue, all or nothing. If it can 500 under concurrent load, the question is whether it also rolls back cleanly under that load, and the concurrency specs that prove it does were passing at the time. Two candidates worth separating before anyone hunts: genuine contention in the charge transaction, versus the harness starving connections when 21 turbo tasks share a pool. The second is a test-environment problem; the first is not. Nobody has looked yet. |
| 64 | **A `deposit_held` leg disagrees with its two neighbours about `member_id`, and it is preserved rather than fixed.** `services/charge.ts` § 7 passes `m.id` on the deposit leg; `depositHeldPosting` and `depositReleasedPosting` pass `null`. `e2e/deposit.test.ts` documents `member_id: NULL` on that account as the invariant its query is built around, and Lane D's activity query joins through `transaction.member_id`, which is why nothing ever caught the disagreement. | Lane A preserved the existing behaviour exactly and pinned it with a spec rather than making a refactor change what a money path writes — which was the right call and the reason to trust the rest of that diff. But now the inconsistency is pinned, so it is a decision rather than a drift: either the deposit leg should carry the member (and `deposit.test.ts`'s documented invariant is wrong) or it should not (and `charge.ts` § 7 is wrong). One of the two is, and both currently ship. |
...
| 58 | **The ledger is never asserted BY ACCOUNT, so a commission leg could post to the wrong side and the whole suite would stay green.** Measured: `avo_commission`, `salon_revenue`, `merchant_bonus_funding` and `gateway_clearing` have **zero occurrences in any test file**; only `member_wallet` appears at all. Every ledger assertion in the suite is a net by direction — `sum(case when direction='credit' then amount_fils else -amount_fils end)` filtered by member (`e2e/adjustments.test.ts:69-74`). A balanced net cannot distinguish `avo_commission` from `salon_revenue`. | **This is the one I would fix first of everything in this row's family.** Swapping those two accounts swaps AVO's revenue with the salon's, the net still balances, and no test names either. It also compounds with the unbuilt reconciliation job (its own ticket): nothing compares our ledger against the processor's, so a misposting is invisible end to end rather than caught at month end. Commission is currently verified only as `topup_intent.fee_fils` — never as its ledger pair. The fix is small: assert the four-row shape a top-up produces, by account, which is exactly what trunk read by hand when validating the demo seed. |
...
| 46 | **`product.nameAr` does not exist, and the customer wallet renders product names in Arabic anyway.** Confirmed: `apps/wallet/src/screens/ShopScreen.tsx:206` renders `p.name` — one name, no variant — so an Arabic customer reads whatever English the merchant typed. `ServiceSchema` has `nameAr`; `ProductSchema` declares four fields and Zod strips a fifth. | Sharper now than when the schema first raised it, because **the Shop editor exists and is the surface that would author the Arabic name** — and it draws one name field. Fixing it needs a widening of trunk-owned `packages/types` (a four-way rebase) *and* a drawn input, so it is a trunk-plus-designer call, not a lane's. Note this does not conflict with `design/README.md` § Known gaps 1: the dashboard stays English-only as a UI; what is missing is an English UI for authoring an Arabic *value* the customer app already displays. |
...
| 38 | **This project has no way to render a component in a test, on any surface.** No `jsdom`, `happy-dom`, `@testing-library` or `react-test-renderer` in any `package.json` — verified across the workspace. The wallet (454 tests), the scanner and the dashboard (245) contain no test that renders anything; every UI assertion is pure-domain logic or SOURCE-TEXT parsing. `brandBootOrder.test.ts` set that idiom by reading `App.tsx` as text, and `payTabWiring.test.ts`, `stateCensus.test.ts` and `consoleNavGates.test.ts` all follow it. | This is the structural reason behind a whole class of finding this session keeps producing: guards that assert what the code SAYS rather than what it DOES. Lane B hit the wall directly — asked for a render test proving the Pay tab shows no suppressed code, it could not write one, and correctly refused to add a renderer because that means `pnpm-lock.yaml` at the repo root, outside any lane's column. Its source-text test is a good substitute and its file header says plainly that a render test would subsume every assertion in it. **The decision is trunk's:** adding a renderer is a root-lockfile change affecting all four surfaces, and it is a real choice (environment per app, and React Native Web vs a native preset for the two Expo apps). Cost of not doing it: every UI guard remains one refactor away from asserting nothing, and the only proof a screen behaves is a human driving it. |
