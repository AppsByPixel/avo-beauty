...
| 72 | **A background worker writes a status transition without asserting the state it believed it was transitioning from.** `services/receiptWorker.ts` `markSent` sets `status='sent'` `WHERE id = $1` — keyed on the row, never on the claim. Found by Lane D the hard way: its `parkOutbox` helper pushes a receipt outside the worker's *claim* predicate, which does nothing about a send already claimed, so claim → park → land silently discards the park. Its suite went red once on `expected [ 'whatsapp' ] to deeply equal [ 'email', 'whatsapp' ]` and green on three standalone re-runs — a phantom flake with a real lost update underneath. | Harmless in production **today**, because nothing but the worker moves those rows, and that is not the reason to fix it: a claim-then-write worker whose write does not check the claim is one concurrent mover away from being wrong, and `WHERE id = $1 AND status = 'sending'` makes the write say what it means. **The inventory matters more than the line** — the same shape may sit in the top-up reaper and the no-show worker, and the severities differ sharply. The reaper consults the payment processor precisely so it never expires an intent the processor calls `succeeded`; if that write is unguarded then the veto is advisory rather than enforced. The no-show worker returns deposits, which is money. Dispatched as an inventory, not a patch. |
