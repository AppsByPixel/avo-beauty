...
| 75 | **`receiptWorker.int.test.ts`'s "does not fail a receipt another worker had already sent" is order-dependent and fails intermittently — and it is why the "int 91 passed" I reported as a clean baseline was luck.** Lane D measured it failing **4 of 5** standalone runs on `avo_lane_d`, with a clean tree, before it changed anything. Trunk reproduced it independently on `avo_lane_c`: **1 of 5**, then **1 of 6**, same assertion every time — `expected 'failed' to be 'sent'` at line 338. The mechanism is verified, not guessed: `soleClaimable` sets both rows' `available_at` in **one** UPDATE (`WHERE id = ANY(...)`), so `now()` evaluates once per statement and the two timestamps **tie**; `claimJobs` is `ORDER BY available_at` with **no tie-break**, so claim order is undefined; and the spec's driver switches on `let first = true` (line 320), assuming racer-then-bystander. When the bystander is claimed first the driver marks the *racer* sent and throws for the *bystander*, which lands in `failed`. | **Lane A's** — `api/src/**/*.int.test.ts` is its column, and Lane D correctly refused to touch it. Fix is either keying the driver off `jobId` instead of call order, or giving the two rows distinct `available_at`. **The reporting failure is mine and is the more useful half.** I ran that suite once, got 91, and published 91 twice — in a merge commit and to Aftab — as the verified state of the branch. A single green run of a suite containing a 20–80% flake is not a measurement, and I had no basis for calling it one. This is the thirteenth instance of this file's recurring shape and the first where the confident unbacked sentence is a **number I generated myself**, which is worse than inheriting a stale comment: nobody had to fail to update it. The rule this earns: **a suite number quoted as a baseline needs more than one run, and a flake found later retroactively invalidates every number previously quoted from that suite.** |
| 74 | **`markSent` takes a `providerReference` and drops it on the floor.** There is no column for it, and `receipts/types.ts` says it is "stored for support to trace" — which is not true today. | Pre-existing and out of the guard slice. Either the parameter or the column should go, and the choice is a product one: if support genuinely needs to trace a receipt back to the provider's id, the column is missing; if not, the parameter and the sentence are both lying. A third instance of this project's recurring shape — a comment asserting a system state that does not exist — this time asserting a capability rather than a gap. |
| 73 | **`TickResult.lost` is now an operational signal, not just a test hook.** The guarded receipt writes report whether they landed, and a persistently non-zero `lost` means two workers are contending faster than the lease intends. `RECEIPT_LEASE_MS` (default 120s) is the knob. | Worth a line in `RUNBOOK.md` beside the receipt worker, because it is the kind of number that means nothing until the day it means everything, and nobody will think to look for it. Also recorded: `releaseHeldCampaigns` has the only other unguarded status write, deliberately left alone — its sole from-state is `approved` with a held reason, the only other writer of campaign status takes `FOR UPDATE` and refuses anything not `pending`, so a second release pass writes the identical value. But **its own comment overstates the `campaign_send` primary key's protection**: with zero recipients or a fully capped-out audience no `campaign_send` row is inserted at all and the PK protects nothing. Same-value write, so nothing is lost — flagged rather than changed. |
