...
| 69 | **`@avo/e2e#test` has no build edge to `@avo/tokens`, so a cold-cache boot can import that package mid-rewrite.** `turbo run test --filter=@avo/e2e --dry=json` shows `@avo/e2e#test ← @avo/mock#build ← @avo/types#build` and nothing else — yet the API the suite boots imports `@avo/tokens` through `routes/salons.ts → services/brandColor.ts`. On a cold cache those run concurrently. | Not #65's cause — it kills a boot rather than a charge — but a genuine cold-cache-only flake source, and this project has now spent two sessions attributing cold-cache flakes to the wrong thing. The fix is one line in `e2e/package.json`, which touches the root lockfile and is therefore outside both Lane D's and Lane A's columns; trunk's to route. Lane A has been asked for a second opinion on the diagnosis before it is acted on. |
